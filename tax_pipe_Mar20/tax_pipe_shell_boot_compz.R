...
# first look at defaults/recommended boots for each algorithm (idtax = 60, bayes = 80, bayes = 50)
# pr2:
...
p1 <- compare_assignments_2way(b80.pr2, i60.pr2,
                               pltfilez = "none",
                               tablenames = c("bayes-pr2, boot=80","idtax-pr2, boot=60"),
                               ranknamez = c("Kingdom", "Supergroup", "Division","Class","Order","Family","Genus","Species"))
...
p.2way.default.bothdb <- plot_grid(
  p1[[3]] + coord_cartesian(ylim = yl),
  p2[[4]] + coord_cartesian(ylim = yl),
  p3[[3]] + coord_cartesian(ylim = yl),
...
ggsave("bayesVidtax_boot_threshold/bothdb_default_boots.pdf", p.2way.default.bothdb, width = 15, height = 12, units = "in", device="pdf")
...
p.lcacompz <- plot_grid(
  ...
  lca.comp.rank[[3]] + coord_cartesian(ylim = yl) + theme(axis.text.x = element_text(angle=45, hjust=1, size=12), plot.margin = unit(c(0.75,0.75,0.75,0.75), "cm")),
  align = 'hv',
  labels = c("A.", "B.", "C.","D."),
  axis = 'l',
  hjust=-1,
...
ggsave("bayesVidtax_boot_threshold/lca_unmapped_pr2vsSilva.pdf", p.lcacompz, width = 3, height = 4, units = "in", device="pdf")
