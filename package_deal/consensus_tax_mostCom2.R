...
# working  on count.na = FALSE below - where assignments are made, need to trim tmp.tblname to only consider tables that have that assignment for lower ranks
# ^think I got it! need to test...
          ...
          if (!trueMajority || max.prop >= threshold) {
            ...
            if (length(c.tax) > 1) {
              ...
              if (!is.na(tiebreaker)) {
                ...
                alltt <- merge(exact, tiebreaker[which(is.na(tiebreaker[,"tax"])), c("table","priority")], by="table", all.x=TRUE)
                
                # resolve priorities
                alltt$priority <- coalesce(alltt$priority.x, alltt$priority.y)
                alltt$priority.x <- NULL
                alltt$priority.y <- NULL
                
                # sort by priority to get tiebreaker at the top of the data frame
                sorted <- alltt[order(alltt$priority), ]
            ...
            if (length(c.tax) > 1) {
              ...
              if (!is.na(tiebreaker)) {
                ...
                if (is.na(sorted[1, "priority"])) {
                  ...
                  c.row[, col] <- sorted[1, "tax"]
                  bob <- data.frame(tblnam = tmp.tblname, tt = alltax[tmp.tblname , col])
                  tina <- as.character(bob[which(bob$tt == c.row[, col]) , "tblnam"])
